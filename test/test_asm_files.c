...
static void test_arith(void)    { ASM_TEST("arith",    asm_path("arith.asm"),    42);     }
static void test_bitwise(void)  { ASM_TEST("bitwise",  asm_path("bitwise.asm"),  0x00FF); }
static void test_loop(void)     { ASM_TEST("loop",     asm_path("loop.asm"),     55);     }
static void test_fib(void)      { ASM_TEST("fib",      asm_path("fib.asm"),      55);     }
static void test_data_ref(void) { ASM_TEST("data_ref", asm_path("data_ref.asm"), 300);    }
static void test_equ(void)      { ASM_TEST("equ",      asm_path("equ.asm"),      1918);   }
static void test_throw(void)    { ASM_TEST("throw",    asm_path("throw.asm"),    0xCAFE); }
static void test_float(void)    { ASM_TEST("float",    asm_path("float.asm"),    42);     }
...
static void test_link_duplicate_global(void) {
    const char *src_a = ".text\nglobal foo\nfoo: halt\n";
    const char *src_b = ".text\nglobal foo\nfoo: halt\n";
...
static void test_file_roundtrip(void) {
    ...
    // Use a temp path that works on both Windows and POSIX
    const char *tmp_path = proj_path("bin/dvm_test_rt2.dvm");
